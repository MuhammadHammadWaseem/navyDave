...
@section('content')
    <div class="col-lg-10">
        <div class="main-calendar-box main-calendar-box-list customers-box">
            <div class="main-table-box main-table-box-list services-table">
                ...
                <div class="form-box">
                    <form action="{{ route('admin.users.session.assign.set',['id'=>$user->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <select name="service" id="service">
                            ...
                            @foreach ($services as $service)
                                <option value="{{ $service->id }}" data-slots="{{ $service->slots }}">{{ $service->name }}
                                </option>
                            @endforeach
                        </select>
    ...
    <script>
        $("#service").on("change", function() {
            // Get the selected option's data-slots attribute
            var slots = $(this).find(":selected").data("slots");

            // Set the slots value to the sessions input field
            $("#session").val(slots);
        });
    </script>
@endsection
